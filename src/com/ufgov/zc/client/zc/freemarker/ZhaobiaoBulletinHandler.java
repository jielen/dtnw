...
public class ZhaobiaoBulletinHandler implements ITemplateToDocumentHandler {
  ...
  private void getxunjia(Map<String, Object> dataMap,ZcEbBulletin bulletin) {
    ...
    dataMap.put("projLinkPhone", StringUtil.freeMarkFillWordChar(bulletin.getZcEbProj().getPhone()));
    dataMap.put("projLinkFax", StringUtil.freeMarkFillWordChar(bulletin.getZcEbProj().getFax()));
    ...
    dataMap.put("bulletinDate", df.format(meta.getTransDate()));   
    ...
    d=bulletin.getZcEbPlan().getSellEndTime()==null?meta.getTransDate():bulletin.getZcEbPlan().getSellEndTime();  
    dataMap.put("sellEndTime", df.format(d));   

    boolean hasAttache=false;
    ...
    for (int i = 0; i < bulletin.getZcEbProj().getPackList().size(); i++) {
      ...
      double sum=pack.getPackBudget()==null?0.0:pack.getPackBudget().doubleValue();
      dataMap.put("packSum" + i, StringUtil.freeMarkFillWordChar(sum+""));
      ...
      if(coName!=null && coName.length()>0 && !namesLst.contains(coName)){
        namesLst.add(coName);
      }
      dataMap.put("coName" + i, coName);
      ...
      for (int j = 0; j < pack.getRequirementDetailList().size(); j++) {
        ...
        row.setPrice(ZcUtil.getDecimalStr(rd.getItemSum()));
        row.setAttachFileName(rd.getItemAttach());
        row.setAttachFileBlobId(rd.getItemAttachBlobid());
        if(rd.getItemAttachBlobid()!=null){
          hasAttache=true;
        }
        rows.add(row);
        if(hasAttache){
          dataMap.put("hasAttache" + i, "1");
        }else{
          dataMap.put("hasAttache" + i, "0");        
        }
      }
