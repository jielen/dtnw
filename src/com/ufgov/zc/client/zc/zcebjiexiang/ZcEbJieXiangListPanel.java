 ...
 * 采购中心项目结项管理
 * 使用于用于采购中心，
 * 一个项目在采购中心中运转时，存在三种情况：正在招标、招标完成、取消采购.
 * 当一个项目完成批办单审批后，即进入正在招标阶段；所有的招标完成之后，进入招标完成状态；在采购进行过程中，如果采购单位要求取消采购，则几in人取消采购状态；
 * 招标完成和取消采购两种采购状态的数据，都需要影响到采购计划，需要释放没有合同额的指标，年终结转时，这类的采购业务，不结转到下一年（不含采购计划、合同部分，因为有未支付完成的合同）
 * 正在招标状态的数据，都需要结转到下一年，继续进行采购；
 * 
 * 这个功能点，目前没有加工作流，方便项目经办人直接操作，主要是年终的时候，有大量的数据要处理，都走审批，不方便。
 * @author Administrator
 *
 */
public class ZcEbJieXiangListPanel extends AbstractEditListBill implements ParentWindowAware {

  private static final Logger logger = Logger.getLogger(ZcEbJieXiangListPanel.class);
  ...
  protected ZcEbJieXiangListPanel self = this;

  private Window parentWindow;

  public String compoId = "ZC_EB_JIE_XIANG";
  ...
  private String listTabId = ZcSettingConstants.TAB_ID_ZC_EB_JIE_XIANG;

  private String listConditionId = ZcSettingConstants.CONDITION_ID_ZC_EB_JIE_XIANG;
  ...
  public IZcEbJieXiangServiceDelegate getZcEbJieXiangServiceDelegate() {
    return zcEbJieXiangServiceDelegate;
  }

  public void setZcEbJieXiangServiceDelegate(IZcEbJieXiangServiceDelegate zcEbJieXiangServiceDelegate) {
    this.zcEbJieXiangServiceDelegate = zcEbJieXiangServiceDelegate;
  }
...
//    toolBar.add(traceDataButton);
  ...
  protected TableModel doExecute() {
    ...
    return mc.convertJieXiangToTableModel(zcEbJieXiangServiceDelegate.getList(elementConditionDto, requestMeta));
  ...
  public void openEditDialog(ArrayList viewList, Integer row, String status) {
    ...
    new ZcEbJieXiangDialog(self, viewList, row, status);
  ...
  private void setButtonStatus() {
    ...
    if (WFConstants.EDIT_TAB_STATUS_DRAFT.equalsIgnoreCase(panelId)) {
      ...
      finishCaiGouButton.setVisible(false);

      unFinishCaigouButton.setVisible(false);

      cancelCaiGouButton.setVisible(false);
      ...
      finishCaiGouButton.setEnabled(false);

      unFinishCaigouButton.setEnabled(false);

      cancelCaiGouButton.setEnabled(false);
    ...
    }else if ("executing".equalsIgnoreCase(panelId)) {

      finishCaiGouButton.setEnabled(true);

      unFinishCaigouButton.setEnabled(false);

      cancelCaiGouButton.setEnabled(true);
    }else if ("wanchengCaiGou".equalsIgnoreCase(panelId)) {

      finishCaiGouButton.setEnabled(false);

      unFinishCaigouButton.setEnabled(true);

      cancelCaiGouButton.setEnabled(false);
    }else if ("quxiaoCaiGou".equalsIgnoreCase(panelId)) {
  ...
  public static void main(String[] args) throws Exception {
    ...
    SwingUtilities.invokeLater(new Runnable() {
      ...
      public void run() {
        ...
        ZcEbJieXiangListPanel bill = new ZcEbJieXiangListPanel();
  ...
  public void refreshCurrentTabData(List dataList) {
    ...
    topDataDisplay.getActiveTableDisplay().getTable().setModel(mc.convertJieXiangToTableModel(dataList));
