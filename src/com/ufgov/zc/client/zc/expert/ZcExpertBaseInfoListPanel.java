...
public class ZcExpertBaseInfoListPanel extends AbstractEditListBill implements ParentWindowAware {
  ...
  public void setZcEmExpertServiceDelegate(IZcEmExpertServiceDelegate zcEmExpertServiceDelegate) {
    this.zcEmExpertServiceDelegate = zcEmExpertServiceDelegate;
  }
        ...
        table.addMouseListener(new MouseAdapter() {
          ...
          public void mouseClicked(MouseEvent e) {
            if (e.getClickCount() == 2 && SwingUtilities.isLeftMouseButton(e)) {
              ...
              getCheckedList();
              new ZcExpertBaseInfoDialog(ZcExpertBaseInfoListPanel.this, viewList, row, tabStatus);
            }
          }
        });
      }
   ...
   * 构造函数
   */
  public ZcExpertBaseInfoListPanel() {
  ...
  private void doDelete() {
    ...
    try {
      for (int i = 0; i < checkedList.size(); i++) {
        EmExpert bean = (EmExpert) checkedList.get(i);
        zcEmExpertServiceDelegate.deleteByPrimaryKey(bean.getEmExpertCode(), requestMeta);
      }
  ...
  private void doCancel() {
    ...
    try {
      for (int i = 0; i < beanList.size(); i++) {
        ...
        emExpert.setEmExpStatus("cancel");
        emExpert.setComment(commentDialog.getComment());
        this.zcEmExpertServiceDelegate.updateByPrimaryKey(emExpert, requestMeta);
      }
    } catch (Exception e) {
      success = false;
    ...
    if (success) {
      JOptionPane.showMessageDialog(this, "作废专家成功！", "提示", JOptionPane.INFORMATION_MESSAGE);
  ...
  private void doUnAudit() {
    ...
    try {
      for (int i = 0; i < beanList.size(); i++) {
        EmExpert emExpert = (EmExpert) beanList.get(i);
        emExpert.setEmExpStatus("suspended");
        this.zcEmExpertServiceDelegate.updateByPrimaryKey(emExpert, requestMeta);
      }
    } catch (Exception e) {
      success = false;
    ...
    if (success) {
      JOptionPane.showMessageDialog(this, "暂停专家成功！", "提示", JOptionPane.INFORMATION_MESSAGE);
      this.refreshCurrentTabData();
    } else {
  ...
  private void doAuditPass() {
    ...
    if (success) {
      JOptionPane.showMessageDialog(this, "启用专家成功！", "提示", JOptionPane.INFORMATION_MESSAGE);
      this.refreshCurrentTabData();
    } else {
  ...
  private void doPrint() {
    List<EmExpert> printList = getCheckedList();
    if (printList.size() == 0) {
  ...
  private void doPrintPreview() {
    final List<EmExpert> printList = getCheckedList();
    if (printList.size() == 0) {
  ...
  public static void main(String[] args) {
    SwingUtilities.invokeLater(new Runnable() {
      public void run() {
        ...
        ZcExpertBaseInfoListPanel bill = new ZcExpertBaseInfoListPanel();
