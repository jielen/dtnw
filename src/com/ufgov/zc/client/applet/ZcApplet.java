...
public class ZcApplet extends JBaseApplet {
  ...
  private String app;
	...
	private User getUser(IBaseDataServiceDelegate baseDataServiceDelegate,
			String userId, RequestMeta requestMeta) {
	...
	private Company getCompany(
			IBaseDataServiceDelegate baseDataServiceDelegate, int nd,
			String coCode, RequestMeta requestMeta) {
		Company company = companyCache.get(coCode);
		if (company == null) {
			company = baseDataServiceDelegate.getCompanyByCoCode(nd, coCode,
					requestMeta);
	...
	private String getOrgPoCode(
			IBaseDataServiceDelegate baseDataServiceDelegate, String coCode,
			String orgCode, String posiCode, int nd, RequestMeta requestMeta) {
		String key = coCode + "-" + orgCode + "-" + posiCode;
		String orgPoCode = orgPoCodeCache.get(key);
		if (orgPoCode == null) {
			orgPoCode = baseDataServiceDelegate.getOrgPosiId(coCode, orgCode,
					poCode, nd, requestMeta);
	...
	private void initWorkEnv() {
				...
				for (Applet oldApplet : oldAppletList) {
					try {
						if (oldApplet instanceof ZcApplet) {
							this.closeOwnedWindows(((ZcApplet) oldApplet)
									.getAppletWindow());
							this.closeOwnerlessWindows(((ZcApplet) oldApplet)
									.getAppletWindow());
						}
						oldApplet.getAppletContext().showDocument(
								new URL("javascript:window.close()"));
		...
		requestMeta.setApp(app);
		requestMeta.setWebIp(webIp);
		requestMeta.setWebRoot(webRoot);
		
		System.out.println("---------set requestMeta=empCode"
				+ requestMeta.getEmpCode());
		...
		java.util.Date sysDate = baseDataServiceDelegate
				.getSysDate(requestMeta);
		workEnv.setSysDate(sysDate);

		if (orgPoCode == null || orgPoCode.trim().equals("")) {
			orgPoCode = getOrgPoCode(baseDataServiceDelegate, coCode, orgCode,
					poCode, workEnv.getTransNd(), requestMeta);
		}
		workEnv.setOrgPoCode(orgPoCode);
		requestMeta.setSvOrgPoCode(orgPoCode);

		IConsoleServiceDelegate consoleServiceDelegate = (IConsoleServiceDelegate) ServiceFactory
				.create(IConsoleServiceDelegate.class, "consoleServiceDelegate");
		List<AsRole> roles = consoleServiceDelegate.getAsRoleByPosi(poCode,
				requestMeta);
		workEnv.setRoles(roles);

		User user = getUser(baseDataServiceDelegate, userId, requestMeta);
		Company company = getCompany(baseDataServiceDelegate,
				workEnv.getTransNd(), coCode, requestMeta);

		workEnv.setCurrUser(user);
		workEnv.setCurrCompany(company);

		if (logger.isDebugEnabled()) {
			logger.debug("workEnv:: user: " + workEnv.getCurrUser().getUserId()
					+ " " + workEnv.getCurrUser().getUserName() + " "
					+ workEnv.getCurrCompany() + " transDate:"
					+ workEnv.getTransDate() + " expertName:"
					+ workEnv.getExpertName() + workEnv.getTransDate()
	...
	private void initAppletParameters() {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("panelClassName: " + panelClassName + " token:"
					+ token + " clientIP:" + clientIP + " userId:" + userId
					+ " cocode:" + coCode + " orgCode:" + orgCode
					+ " orgPocode: " + orgPoCode + " poCode:" + poCode
					+ " webRoot:" + webRoot + " transDate: " + transDate
					+ " accountId:" + expertCode + " : " + expertCode
					+ " expertName:" + expertName + "empCode:" + empCode
					+ "empName:" + empName
          + "app:" + app);
		}

		System.out.println("panelClassName: " + panelClassName + " token:"
				+ token + " clientIP:" + clientIP + " userId:" + userId
				+ " cocode:" + coCode + " orgCode:" + orgCode + " orgPocode: "
				+ orgPoCode + " poCode:" + poCode + " webRoot:" + webRoot
				+ " webIp:" + webIp + " transDate: " + transDate
				+ " accountId:" + expertCode + " : " + expertCode
				+ " expertName:" + expertName + "empCode:" + empCode
				+ "empName:" + empName
        + "app:" + app);
	...
	private void initPanel(Container container) {
		try {
			...
			String preferredFontSize = AsOptionMeta
					.getOptVal(SystemOptionConstants.OPT_PREFERRED_FONT_SIZE);
	...
	public String getParameter(String name) {
		if ("lookAndFeel".equals(name)) {
			...
			String v = lookAndFeelMap.get(AsOptionMeta
					.getOptVal(SystemOptionConstants.OPT_SYS_LOOK_AND_FEEL));
			if (v != null) {
				className = v;
			}
			return className;
	...
	public int hashCode() {
		...
		result = prime * result
				+ ((appletWindow == null) ? 0 : appletWindow.hashCode());
		result = prime * result
				+ ((clientIP == null) ? 0 : clientIP.hashCode());
		result = prime * result + ((coCode == null) ? 0 : coCode.hashCode());
		result = prime * result + ((orgCode == null) ? 0 : orgCode.hashCode());
		result = prime * result
				+ ((orgPoCode == null) ? 0 : orgPoCode.hashCode());
		result = prime * result
				+ ((panelClassName == null) ? 0 : panelClassName.hashCode());
		...
		result = prime * result
				+ ((expertCode == null) ? 0 : expertCode.hashCode());
		result = prime * result
				+ ((expertName == null) ? 0 : expertName.hashCode());
